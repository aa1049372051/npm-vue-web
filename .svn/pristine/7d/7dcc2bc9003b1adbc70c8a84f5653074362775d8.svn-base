    ...
    <el-form :model="ruleForm" ref="ruleForm" inline size="">
      <div class="in-block pad-bot4" style="width120">
        ...
        >
          <el-select
            :class="areaClass.p || {}"
            v-model="ruleForm.myProvinceID"
            :disabled="disabled || provinceDisabled || disabledRank > 0"
            :clearable="clearable"
            ref="province"
      ...
      <div class="in-block pad-bot4" style v-if="rank != 1">
        ...
        >
          <el-select
            :class="areaClass.c || {}"
            :disabled="cDisabled || disabled || disabledRank > 1"
            v-model="ruleForm.myCityID"
            ref="city"
            :placeholder="emptyStr[1]"
          >
            <el-option
              :key="0"
              label="全部"
              :value="0"
              v-if="showEmpty && cityList && cityList.length > 0"
            />
            <el-option
      ...
      <div class="in-block mar-r0 pad-bot4" v-if="rank == 3">
        ...
        >
          <el-select
            :class="areaClass.a || {}"
            :disabled="aDisabled || disabled || disabledRank > 2"
            v-model="ruleForm.myAreaID"
            ref="area"
            :placeholder="emptyStr[2]"
            @change="keyUp"
          >
            <el-option
              :key="0"
              label="全部"
              :value="0"
              v-if="showEmpty && areaList && areaList.length > 0"
            />
            <el-option
...
export default {
  ...
  methods: {
    ...
    getCity(id) {
      ...
      let cityList = sessionStorage.getItem(id)
      ...
      if (cityList && Object.keys(cityList).length) {
        ...
        if (
          oldCityID &&
          parseInt(oldCityID / 10000) == parseInt(cityList[0]["id"] / 10000)
        ) {
          //如果是同一个省份的城市
          newCityID = oldCityID;
      ...
      axios.post("pqs/are001", { grade: 3, parentID: id }).then(
        (response) => {
          ...
          if (response.data.header.success) {
            cityList = response.data.body.result.areaList || [];
            if (cityList) {
    ...
    getArea(id) {
      ...
      let areaList = sessionStorage.getItem(id)
        ? JSON.parse(sessionStorage.getItem(id))
        : [];
      ...
      if (areaList && Object.keys(areaList).length) {
        ...
        if (
          oldAreaID &&
          parseInt(oldAreaID / 100) == parseInt(areaList[0]["id"] / 100)
        ) {
          newAreaID = oldAreaID;
      ...
      axios.post("pqs/are001", { grade: 4, parentID: id }).then(
        (response) => {
          this.aDisabled = false;
          if (response.data.header.success) {
            areaList = response.data.body.result.areaList || [];
            if (areaList) {
    ...
    getName() {
      ...
      this.aTimeOut = setTimeout(() => {
        ...
        if (this.rank != 1)
          this.$emit("update:cityName", this.$refs.city.selectedLabel);
        if (this.rank == 3)
          this.$emit("update:areaName", this.$refs.area.selectedLabel);
        this.$emit("change");
      }, 100);
    },
    getData() {
    ...
    getProvince() {
      ...
      if (!provinceList) {
        ...
        axios
          ...
          .then((response) => {
            if (response.data.header.success) {
              let res = response.data.body.result.areaList || [];
              res.forEach((element) => {
