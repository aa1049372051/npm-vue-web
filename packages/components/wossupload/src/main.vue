<template>
  <div :class="divWidth ? 'width100' : ''">
    ...
    <div class="en-clo-li fl wid90 mar-top10" v-if="showImgFile && !customList">
      ...
      <ul class="clear">
        <li v-for="(item, index) in newFile" :key="item.keyurl + '' + index">
          ...
          <template v-else>
            ...
            >
              ...
              <img
                v-else
                style="width: 50px; height: 50px"
                @click="$emit('imgClick', item.myPath)"
                :src="item.myPath"
              />
            </div>
          </template>
  ...
  methods: {
    ...
    upSuccess(response, file) {
      ...
      this.fileList.push({
        ...
        keyurl: this.myFile.isKey
          ? this.form.key + "?key=" + new Date().getTime()
          : this.form.key,
        size: file.size,
        type: file.raw.type,
    ...
    uploadBase64Img(base64Content, filename, cb, type) {
      if (this.fileList.length >= this.fileCount) {
        this.$alert(`最多只能上传${this.fileCount}个文件`);
        return false;
      }
      if (type == "img" || base64Content.indexOf("data:image") === 0) {
        let imgfile = null;
        if (type == "img") {
          imgfile = base64Content;
          console.log(base64Content);
        } else {
          imgfile = this.dataURLtoFile(base64Content, "img.png");
        }
        this.updisabled = true;
        this.isStart = true;
        let filekey = this.getScreenShotKey();
        this.ossclient
          .multipartUpload(filekey.key, imgfile, {})
          .then((res) => {
            this.updisabled = false;
            this.isStart = false;
            this.fileList.push({
              name: filename || filekey.name,
              url: this.upUrl + "/" + res.name,
              keyurl: res.name,
              size: imgfile.size || 0,
              type: "png",
              fileType: 1,
              fileName: filename || filekey.name,
              filePath: res.name,
              fileSize: imgfile.size || 0,
            });
        ...
        console.error("Please fill in the correct Base64 img");
      }
    },
    setOSSClient(data) {
  ...
  computed: {
    ...
    newFile() {
      ...
      return fileList.filter((item) => {
        if (!item.randomKey) {
          item.randomKey = Math.random() + "" + item.keyurl;
        }
